...
FROM (
		SELECT		
			...
			l.delivery_date + l.weeks * interval '7 day' AS return_date,
		...
		WHERE	l.was_accepted IS TRUE AND l.gamer_status_date IS NOT NULL
		UNION
		...
		SELECT	
			...
			lr.delivery_date AS delivery_date,
			lr.delivery_date + lr.weeks * interval '7 day' AS return_date,
			lr.payment_date AS payment_date
